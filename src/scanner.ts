...
const VALID_CATEGORIES: Record<string, true> = {
	general: true,
	minor: true,
	sexual: true,
	belittle: true,
...
export function validateCategories(categories: readonly string[]): void {
	for (const c of categories) {
 if (!VALID_CATEGORIES[c]) throw new RangeError(`Unknown category: ${c}`)
	}
}

const URL_REGEX = /https?:\/\/\S+|www\.\S+/g

export function buildUrlSpans(text: string): Array<{ start: number; end: number }> {
...
const multiCharRaw = replacements.multiChar as Array<{ from: string; to: string }>
...
const multiCharRegex = new RegExp(
	multiCharEntries
...
function runMultiCharPass(entries: Entry[]): Entry[] {
	...
	for (let m = matches.length - 1; m >= 0; m--) {
		...
		const covering = findCoveringEntries(
			result,
			match.index!,
...
const singleCharMap = replacements.singleChar as Record<string, string>

function runSingleCharPass(entries: Entry[]): Entry[] {
	for (let i = 0; i < entries.length; i++) {
		...
		if (newText !== e.text) entries[i] = { ...e, text: newText }
	}
	return entries
}

const englishSingleCharMap = replacements.englishSingleChar as Record<string, string>

function runEnglishSingleCharPass(entries: Entry[]): Entry[] {
	for (let i = 0; i < entries.length; i++) {
		const e = entries[i]!
		let newText = ""
		for (const c of e.text) {
...
function runWhitespaceCollapsePass(entries: Entry[]): Entry[] {
	const result: Entry[] = []
	let i = 0
...
const categoryRewrites = replacements.categoryRewrites as Record<
	string,
	Array<{ from: string; to: string }>
>

function runRewritePass(entries: Entry[], category: string): Entry[] {
	const rewrites = categoryRewrites[category]
	if (!rewrites) return entries
...
const koFPData = fpData.ko as Record<string, string[]>

const fpRegexes = new Map<string, RegExp | null>()
...
function runFPPass(entries: Entry[], category: string): Entry[] {
...
const finalFilterRanges = replacements.finalFilterRanges as Record<string, string>

const finalFilterRegexes = new Map<string, RegExp | null>()
for (const [cat, ranges] of Object.entries(finalFilterRanges)) {
	finalFilterRegexes.set(cat, new RegExp(`[^${ranges}]`, "gu"))
}

function runFinalFilterPass(entries: Entry[], category: string): Entry[] {
	const regex = finalFilterRegexes.get(category)
	if (!regex) return entries
	const result: Entry[] = []
	for (const e of entries) {
		const filtered = e.text.replace(regex, "")
		if (filtered.length > 0) result.push({ ...e, text: filtered })
	}
	return result
}

const koJson = koData as {
	general: string[]
	minor: string[]
	sexual: string[]
	belittle: string[]
...
function getCategoryPatterns(category: string): string[] {
	const base = (koJson as unknown as Record<string, string[]>)[category] ?? []
...
const koreanPatternRegexes = new Map<string, RegExp>()
...
const englishPatterns = enData as string[]

function buildEnglishPattern(entry: string): string {
	let mapped = ""
	...
	let pattern = escaped
	if (startsAlphanum) pattern = `(?<![a-z0-9])${pattern}`
	if (endsAlphanum) pattern = `${pattern}(?![a-z0-9])`
	return pattern
}

const englishPatternRegex = new RegExp(
	[...englishPatterns]
